...
class RSIFVG(IStrategy):
    ...
    min_lookback = IntParameter(
        2, 10, default=5, space="indicator"
    )  # in pivot counts (rsi_timeframe bars)
    max_lookback = IntParameter(
        20, 80, default=60, space="indicator"
    )  # in pivot counts (rsi_timeframe bars)
    divergence_expiry = IntParameter(
        4, 20, default=20, space="indicator"
    )  # rsi_timeframe periods before divergence invalid

    fvg_threshold = DecimalParameter(
        0.0, 0.01, default=0.0, decimals=4, space="indicator"
    )  # min relative gap size (used when auto_threshold=False)
    fvg_extend = IntParameter(
        10, 200, default=20, space="indicator"
    )  # how many fvg_timeframe bars FVG stays active
    ...
    def informative_pairs(self):
        ...
        pairs = [(p, self.rsi_timeframe) for p in wl] + [(p, self.fvg_timeframe) for p in wl]
        return pairs

    # --- Helper: FVG detection ---
    @staticmethod
    def _detect_fvgs(
        df: DataFrame, threshold: float, auto_threshold: bool, fvg_extend: int
    ) -> DataFrame:
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        inf_rsi["price_pivot"] = self._pivot(
            inf_rsi["close"], int(self.pivot_left.value), int(self.pivot_right.value)
        )
        inf_rsi["rsi_pivot"] = self._pivot(
            inf_rsi["rsi"], int(self.pivot_left.value), int(self.pivot_right.value)
        )
        ...
        logger.info(
            f"{pair} FVG Detection ({self.fvg_timeframe}): {bull_fvgs_detected} bullish, {bear_fvgs_detected} bearish FVGs found | "
        ...
        dataframe = merge_informative_pair(
            dataframe, inf_rsi, self.timeframe, self.rsi_timeframe, ffill=True
        )
        dataframe = merge_informative_pair(
        ...
        dataframe["in_bull_fvg"] = (
            dataframe[f"in_bull_fvg_{self.fvg_timeframe}"].fillna(0).astype(int)
        )
        dataframe["in_bear_fvg"] = (
            dataframe[f"in_bear_fvg_{self.fvg_timeframe}"].fillna(0).astype(int)
        )
        ...
        expiry_minutes = int(self.divergence_expiry.value) * timeframe_to_minutes(
            self.rsi_timeframe
        )
        ...
        recent_pivot_lows: List[
            Dict[str, Any]
        ] = []  # {'t': timestamp, 'price': float, 'rsi': float}
        ...
        for idx, row in dataframe.iterrows():
            ...
            if row.get(price_piv) == -1 or row.get(rsi_piv) == -1:
                ...
                if not recent_pivot_lows or recent_pivot_lows[-1]["t"] != t:
                    recent_pivot_lows.append(
                        {"t": t, "price": row["close_" + self.rsi_timeframe], "rsi": row[rsi_col]}
                    )
            if row.get(price_piv) == 1 or row.get(rsi_piv) == 1:
                # Only add if this is a distinct pivot (not duplicate)
            ...
            if len(recent_pivot_lows) >= 2:
                ...
                if (
                    ...
                    and span_minutes >= min_range_minutes
                ):
            ...
            dataframe.at[idx, "bull_div_active"] = (
                1
                if last_bull_div_time
                and (t - last_bull_div_time).total_seconds() / 60 <= expiry_minutes
                else 0
            )
            dataframe.at[idx, "bear_div_active"] = (
                1
                if last_bear_div_time
                and (t - last_bear_div_time).total_seconds() / 60 <= expiry_minutes
                else 0
            )
        ...
        logger.info(
            f"{pair} Divergence Detection ({self.rsi_timeframe}): {bull_divs} bull divs, {bear_divs} bear divs detected | "
        ...
        logger.info(
            f"{pair} Base Timeframe FVG Status: {in_bull_fvg_count} candles in bull FVG, {in_bear_fvg_count} candles in bear FVG"
        )
        ...
        logger.info(
            f"{pair} Engulfing Patterns ({self.timeframe}): {bull_engulf_count} bullish, {bear_engulf_count} bearish"
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if len(dataframe) > 0:
            ...
            long_entries = (
                (dataframe["enter_long"] == 1).sum() if "enter_long" in dataframe.columns else 0
            )
            short_entries = (
                (dataframe["enter_short"] == 1).sum() if "enter_short" in dataframe.columns else 0
            )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe["bear_div_active"] == 1), ["exit_long", "exit_tag"]] = (
            1,
            "bear_divergence_signal",
        )

        # Exit short positions when bullish divergence appears
        dataframe.loc[(dataframe["bull_div_active"] == 1), ["exit_short", "exit_tag"]] = (
            1,
        ...
        **kwargs,
        ...
        if current_profit > initial_sl * 4.0:
            return stoploss_from_open(
                initial_sl * 3.0, current_profit, is_short=trade.is_short, leverage=trade.leverage
            )

        # At 3R profit, trail stop at 2R (lock in 2R profit)
        elif current_profit > initial_sl * 3.0:
            return stoploss_from_open(
                initial_sl * 2.0, current_profit, is_short=trade.is_short, leverage=trade.leverage
            )

        # At 1.5R profit, move to breakeven (lock in entry price)
        elif current_profit > initial_sl * 1.5:
            return stoploss_from_open(
                0.001, current_profit, is_short=trade.is_short, leverage=trade.leverage
            )
