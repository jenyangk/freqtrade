...
class EMAFVG(IStrategy):
	...
	fvg_threshold = DecimalParameter(0.0, 0.01, default=0.0, decimals=4, space='indicator')  # min relative gap size
	fvg_extend_15m = IntParameter(20, 100, default=50, space='indicator')  # how many 15m periods FVG stays active
	...
	def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
		...
		if len(dataframe) > 0:
			...
			if self.custom_info[pair].get('last_indicator_hash') == indicator_hash and \
				{'in_bull_fvg','in_bear_fvg','bull_engulf','bear_engulf'}.issubset(dataframe.columns):
				return dataframe
		...
		dataframe = merge_informative_pair(dataframe, inf_ema, self.timeframe, self.ema_timeframe, ffill=True)
		...
		new_bull_rows = inf_fvg[inf_fvg['fvg_bull_top'].notna()][['date', 'fvg_bull_top', 'fvg_bull_bottom']]
		for _, r in new_bull_rows.iterrows():
			ts = r['date']
			if ts not in seen_bull:
				active_fvgs.append({'t': ts, 'top': r['fvg_bull_top'], 'bottom': r['fvg_bull_bottom'], 'bull': True, 'filled': False})
				seen_bull.add(ts)
		new_bear_rows = inf_fvg[inf_fvg['fvg_bear_top'].notna()][['date', 'fvg_bear_top', 'fvg_bear_bottom']]
		for _, r in new_bear_rows.iterrows():
			ts = r['date']
			if ts not in seen_bear:
				active_fvgs.append({'t': ts, 'top': r['fvg_bear_top'], 'bottom': r['fvg_bear_bottom'], 'bull': False, 'filled': False})
				seen_bear.add(ts)
		...
		active_fvgs = [f for f in active_fvgs if (cutoff is None or f['t'] >= cutoff) and not f.get('filled', False)]
		...
		for idx, row in dataframe.iterrows():
			...
			for f in active_fvgs:
				...
				beyond = (f['bull'] and price_close < f['bottom']) or (not f['bull'] and price_close > f['top'])
	...
	def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float,
		...
		if len(dataframe) > 0:
			...
			if is_long and last_candle.get('engulf_swing_low', 0) > 0:
				...
				if pair not in self.custom_info:
					self.custom_info[pair] = {'trade_stoplosses': {}, 'pending_entry': None, 'active_fvgs': [], 'fvg_seen_bull': set(), 'fvg_seen_bear': set()}
