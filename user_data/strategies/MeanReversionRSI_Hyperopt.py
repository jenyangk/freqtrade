...
class MeanReversionRSI_Hyperopt(IStrategy):
    """
    MeanReversionRSI Strategy based on Jesse Trade strategy - Hyperopt Version.
    
    Logic:
    - Long when RSI(4h) > rsi_long_threshold, Supertrend(4h) is UP, ADX > adx_threshold, ADX(4h) > adx_4h_threshold.
    - Short when RSI(4h) < rsi_short_threshold, Supertrend(4h) is DOWN, ADX > adx_threshold, ADX(4h) > adx_4h_threshold.
    - Entry: Limit order at BB Lower (Long) or BB Upper (Short).
    - Exit: Take Profit at BB Upper (Long) or BB Lower (Short).
    - Stop Loss: Fixed at Entry +/- atr_sl_multiplier * ATR.
    """
    INTERFACE_VERSION = 3
    ...
    # RSI
    rsi_long_threshold = IntParameter(60, 80, default=70, space="buy")
    rsi_short_threshold = IntParameter(20, 40, default=30, space="buy")
    
    # ADX
    adx_threshold = IntParameter(10, 30, default=20, space="buy")
    adx_4h_threshold = IntParameter(20, 50, default=40, space="buy")
    ...
    # Bollinger Bands
    bb_window = IntParameter(15, 25, default=20, space="buy")
    bb_std = DecimalParameter(1.5, 2.5, default=2.0, space="buy")
    
    # ATR Stoploss Multiplier
    atr_sl_multiplier = DecimalParameter(3.0, 8.0, default=6.0, space="sell")
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        # Stoploss is atr_sl_multiplier * ATR
        sl_distance = self.atr_sl_multiplier.value * atr
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Use hyperopt parameters for BB
        bollinger = ta.BBANDS(dataframe, timeperiod=self.bb_window.value, nbdevup=self.bb_std.value, nbdevdn=self.bb_std.value, matype=0)
        ...
        # Supertrend 4h
        # Use hyperopt parameters for Supertrend
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float,
        ...
        if len(dataframe) > 0:
            ...
            sl_price = 0.0
            if side == "long":
                sl_price = rate - (atr * self.atr_sl_multiplier.value)
            else:
                sl_price = rate + (atr * self.atr_sl_multiplier.value)
